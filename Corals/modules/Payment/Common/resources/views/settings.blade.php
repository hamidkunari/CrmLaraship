                            ...
                            <div id="{{ $gateway_key }}"
                                ...
                                @foreach($gateway['settings'] as $key => $setting)
                                    ...
                                    @elseif($setting['type'] == 'boolean')
                                        {!! CoralsForm::boolean($gateway_key.'_'.$key,$setting['label'],false,\Settings::get($gateway_key.'_'.$key)) !!}
                                    @elseif($setting['type'] == 'textarea')
                                        {!! CoralsForm::textarea($gateway_key.'_'.$key,$setting['label'],false,\Settings::get($gateway_key.'_'.$key)) !!}
                                    @elseif($setting['type'] == 'select')
                                        {!! CoralsForm::select($gateway_key.'_'.$key,$setting['label'],$setting['options'],false,\Settings::get($gateway_key.'_'.$key)) !!}
                                    @endif
                                @endforeach
